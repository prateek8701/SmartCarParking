...
export function ParkingGrid() {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3">
      {slots.map((slot) => (
        <SlotCard key={slot.id} slot={slot} onClick={() => toggleSlotStatus(slot.id)} />
      ))}
    </div>
  );
}

function SlotCard({ slot, onClick }: { slot: ParkingSlot; onClick: () => void }) {
  const statusColors = {
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        slot.status === 'free' ? "bg-green-500/10" : 
        slot.status === 'occupied' ? "bg-red-500/10" : 
        "bg-amber-500/10"
      )}>
